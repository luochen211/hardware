...
先认识一套联网硬件怎样感知环境、传递信息并控制动作，再按兴趣阅读芯片、职业和投入结构。后续专题文档继续收录在 `docs/`，并从这里增加入口。

![联网硬件三层概念图：感知层、网络层、应用层，以及数据上报和控制命令的双向流动](docs/images/iot-three-layer-architecture.png)

| 专题 | 从中了解什么 |
| --- | --- |
| [硬件是什么：领域与系统导览](docs/硬件宏观认知思维导图.md) | 感知、控制、连接和验证怎样组成一套硬件系统 |
| [芯片从设计到流片](docs/芯片从设计到流片.md) | 芯片、单片机和开发板的区别，以及设计、验证、流片与制造流程 |
| [硬件与软件：入行门槛和经验积累](docs/硬件与软件职业路径对比.md) | 两类工作的试错方式、迭代速度与经验积累 |
| [硬件、软件与 AI：投入发生在哪里](docs/硬件软件与AI时代的投入结构.md) | 原型、规模部署和持续运营各阶段的成本 |

电脑或手机收到设备上报的数据，只能说明设备发送了该读数；传感器接线、供电和测量准确性，还需要诊断信息与实物验证。
