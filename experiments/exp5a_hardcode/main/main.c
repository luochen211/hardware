/**
 * ============================================================
 *  实验五：WiFi STA 连接 + TCP 客户端通信 + JSON 数据上报
 * ============================================================
 *  功能：
 *    1. 连接 WiFi（STA 模式）
...
static void tcp_client_task(void *arg)
{
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Socket 创建成功，正在连接 %s:%d ...", TCP_SERVER_IP, TCP_SERVER_PORT);
        ...
        if (err != 0) {
            ...
            close(sock);
            vTaskDelay(pdMS_TO_TICKS(5000));   /* 等5秒后重连 */
            continue;
        }
...
void app_main(void)
{
    ESP_LOGI(TAG, "=== 实验五：WiFi STA + TCP 客户端 ===");
    ...
    /* 2. OLED初始化 */
    oled_init();
    oled_show_text(0, "Exp5 TCP Client");
