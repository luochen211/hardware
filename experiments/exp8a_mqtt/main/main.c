                                ...
                                int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ESP_LOGI(TAG, "收到: %.*s = %.*s", event->topic_len, event->topic, event->data_len, event->data);
        {
            ...
            } else if (strstr(buf, "fan_on")) {
                gpio_set_level(FAN_GPIO, 1);
                ESP_LOGI(TAG, "Fan ON");
            } else if (strstr(buf, "fan_off")) {
...
static void sensor_task(void *arg)
{
    ...
    while (1) {
        ...
        snprintf(payload, sizeof(payload),
                 "{\"temp\":%.1f,\"humi\":%.1f,\"seq\":%d}", temp, humi, seq);
...
void app_main(void)
{
    ...
    ws2812_set_color(50, 0, 0);
    oled_init();
    oled_show_text(0, "Smart Farm v1.0");
    ...
    if (gpio_get_level(BUTTON_K1C) == 0) {
        oled_show_text(0, "Reset WiFi?");
        vTaskDelay(pdMS_TO_TICKS(5000));
        if (gpio_get_level(BUTTON_K1C) == 0) {
            ESP_LOGI(TAG, "清除WiFi凭据");
            oled_show_text(0, "Resetting...");
            esp_wifi_restore();
            nvs_flash_erase();
            esp_restart();
        }
    ...
    /* WiFi */
    oled_show_text(0, "Connecting...");
    wifi_init();
    oled_show_text(0, "WiFi OK!");
    ws2812_set_color(0, 50, 0);

    /* MQTT */
    oled_show_text(3, "Connecting MQTT");
    mqtt_start();
    ...
    ESP_LOGI(TAG, "控制: %s", TOPIC_CMD);
    ESP_LOGI(TAG, "  led_on / led_off - 控制LED");
    ESP_LOGI(TAG, "  fan_on / fan_off - 控制风扇");
}
